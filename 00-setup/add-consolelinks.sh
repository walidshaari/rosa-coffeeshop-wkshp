...
# Add DevSpace link.
echo "  href: https://devspaces.$apps#http://gitea.scm.svc.cluster.local:3000/developer/coffeeshop/raw/branch/main/devfile.yaml" \
  >> console-links/coffee-devspace.yaml
...
# Add Prod Grafana link.
echo "  href: https://grafana-route-workload-monitoring.$apps" \
  >> console-links/grafana.yaml

# Add Prod Grafana link.
#echo "  href: https://grafana-route-workload-monitoring.$apps" \
#  >> console-links/grafana.yaml

oc create -f console-links/coffeeshop-devspace.yaml
...
oc create -f console-links/grafana.yaml
